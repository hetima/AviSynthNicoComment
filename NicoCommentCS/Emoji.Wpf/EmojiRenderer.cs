...
namespace NCEmoji.Wpf
{
    ...
    {
        static System.Windows.Media.Brush FallbackBrush = System.Windows.Media.Brushes.White;

        public static BitmapSource RenderBitmap(string text, double font_size)
        {
            var key = new RenderKey() { Text = text, FontSize = font_size };
        ...
        public static BitmapSource RenderBitmapInternal(string text, double font_size)
        {
            ...
            var scale = font.GetScale(font_size) * 0.75; // 72/96?
            ...
            if (glyphplansequence.Count > 0 && width > 0 && height > 0)
            {
                ...
                for (int i = 0; i < glyphplansequence.Count; ++i)
                {
                    ...
                    font.RenderGlyph(dc, g.glyphIndex, origin, font_size, FallbackBrush);
